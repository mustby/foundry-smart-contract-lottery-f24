...
deploy-sepolia :
	@forge script scripts/DeployRaffles.s.sol:DeployRaffle --rpc-url $(SEPOLIA_RPC_URL)
	--account default --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv

deploy-anvil :
	@forge script scripts/DeployRaffles.s.sol:DeployRaffle --rpc-url $(ANVIL_RPC_URL)
	--account default --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv